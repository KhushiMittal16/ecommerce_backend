...
exports.listSearch = async (req, res) => {
  ...
  const allPost = await ProductNew.find(queryname).lean();
  res.json(allPost);
};
...
exports.listByCategory = async (req, res) => {
  ...
  const query = {};
  // console.log("req");
  const allPost = await ProductNew.find({
...
exports.decreaseQuantity = (req, res, next) => {
  // console.log("product in cart:",req.body)
  ...
  ProductNew.bulkWrite(bulkOps, {}, (error, products) => {
    if (error) {
      return res.status(400).json({
